...
- **Ownership split** (settled during ticket 02) — framework owns the desk page shell, the mount contract, the build preset and the renderer toggle; Insights provides doctypes, engine, and an island it builds and bundles itself. The seam is one call: framework's page asks Insights to mount into an element with desk context. The Insights→desk bridge (is Insights installed? is the flag on? is this an Insights dashboard?) lives in framework, so Insights never knows about the fallback.
- **An app bundles its own island.** Supersedes the runtime half of tickets 02, 07 and 08. There is no framework runtime artifact, no import map and no shared runtime stylesheet. An island is a self-contained ES module carrying its own Vue, vue-router and frappe-ui, and an app ships one stylesheet for all its islands. Framework owns a name, a URL and a `mount(el, context)` export, so nothing has to be on the page before an island loads. The seven decisions are in `apps/frappe/ui/island/decisions/`; the mount contract is `@framework/ui/island` and the build preset `@framework/ui/vite/island`. This also dissolves [ticket 28](issues/28-runtime-version-authority.md): one bundle holds one copy of each package, so no second lockfile arbitrates a version.

The framework-side foundation is specced from these tickets:
[spec-framework-foundation.md](spec-framework-foundation.md) (`shipped`, with its
runtime design superseded — see the entry above).
