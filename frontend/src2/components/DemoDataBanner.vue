...
<template>
	<div
		v-if="show"
		class="flex flex-col gap-3 rounded-lg bg-surface-white px-3 py-2.5 text-sm shadow-sm"
	>
		<div class="flex items-start justify-between">
			<div class="flex flex-col gap-1">
				<div class="font-medium text-p-base text-ink-gray-9">{{ __('Try demo data') }}</div>
			...
			<button
				v-if="!loading"
				class="mt-0.5 shrink-0 rounded p-0.5 text-ink-gray-4 hover:text-ink-gray-6"
				@click="dismiss"
			>
				<X class="h-3.5 w-3.5" />
			</button>
