	...
	columns: [
		...
		},
		{
			...
			prefix: (props: any) => {
				...
				if (user.invitation_status) {
					color =
						user.invitation_status === 'Pending' ? 'text-yellow-500' : 'text-ink-red-3'
				} else {
					color = props.row.enabled ? 'text-green-500' : 'text-ink-gray-4'
				}
				...
				<div class="flex flex-wrap gap-1 rounded bg-surface-gray-2 p-0.5">
					...
					<div class="min-w-[10rem] flex-1">
						<input
							...
							class="h-7 w-full rounded border-none bg-surface-gray-2 py-1.5 pl-2 pr-2 text-base text-ink-gray-8 placeholder-gray-500 transition-colors focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0"
						/>
					</div>
				</div>
			</div>
